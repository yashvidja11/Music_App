  ...
  return (
    <section>
      <div className="container px-20 ">
        <h1 className="text-5xl mb-10 font-extrabold text-white">
          Music Playlists
        </h1>
        {isLoading && (
        ...
        <Slider {...settings}>
          {data?.playlists?.map((item) => (
            <div
              key={item.id}
              className="bg-white p-4 shadow rounded-lg flex flex-col justify-between transition duration-300 transform hover:-translate-y-1 hover:shadow-lg h-full"
            >
              <div className="flex flex-col justify-between h-full">
                <Card
                  title={`${item.title}`}
                  imgurl={`${item?.image[2]?.link}`}
                />
